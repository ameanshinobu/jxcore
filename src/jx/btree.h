...
struct btree_is_key_compare_to
  : public is_convertible<Compare, btree_key_compare_to_tag> {
};
...
struct btree_set_params
  : public btree_common_params<Key, Compare, Alloc, TargetNodeSize,
  sizeof(Key)> {
    typedef false_type data_type;
    typedef false_type mapped_type;
...
public:
  ...
  typedef typename if_<
  is_integral<key_type>::value ||
  is_floating_point<key_type>::value,
...
struct btree_iterator {
  ...
  typedef typename remove_const<Node>::type normal_node;
...
void btree_node<P>::swap(btree_node *x) {
  ...
  int n = (count() > x->count())?count():x->count();
  for (int i = 0; i < n; ++i) {
    value_swap(i, x, i);
  }
  for (int i = count(); i < x->count(); ++i) {
...
bool btree<P>::try_merge_or_rebalance(iterator *iter) {
  ...
  if (iter->node->position() < parent->count()) {
    ...
    if ((right->count() > kMinNodeValues) &&
      ...
      to_move = (to_move < right->count() - 1)?(to_move):(right->count() - 1);
  ...
  if (iter->node->position() > 0) {
    ...
    if ((left->count() > kMinNodeValues) &&
      ...
      to_move = (to_move < left->count() - 1)?to_move:(left->count() - 1);
